...
#####Here is the make tile gram path 
  sp <- sp::spTransform(shp, sp::CRS("+proj=longlat +ellps=clrk66"))
  tiles <- hex_tiles(sp)
  tiles <- tiles[sp, ]
  ...
  cents <- SpatialPointsDataFrame(coords=shp_cents, data=shp@data, 
  ...
  if(is.character(cents$pop_field)==TRUE){
    ...
    warning("Pop. field was character and converted to numeric.")
  ...
  if(missing(jenks)==FALSE | jenks==TRUE ){
  ...
  if(missing(label_field)==FALSE){
    text(data$xcor+ 0.1,data$ycor + 0.1,labels=data$label, cex=0.4)
  }
  return(data)
